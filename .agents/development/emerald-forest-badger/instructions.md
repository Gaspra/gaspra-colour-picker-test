...
- [ ] Create `index.html` with clean semantic markup (including `<main>`, `<section>`, and `<header>` tags), unique IDs for all interactive elements, and a viewport meta tag.
- [ ] Set up Google Font imports in `style.css` (e.g., **Outfit** or **Plus Jakarta Sans** for headers/labels, and **JetBrains Mono** or **Fira Code** for monospaced values).
- [ ] Implement the core HSL/HSV color mapping in `app.js`:
...
- [ ] Design and style the premium center HUD card in `style.css` using glassmorphism:
...
- [ ] Update the HUD dynamically with HEX, RGB, and HSL text readouts and a color preview indicator at 60fps.
- [ ] Implement adaptive HUD text/border color: calculate the relative luminance of the current background color (using the standard WCAG relative luminance formula) and dynamically toggle the HUD's text/border class between light and dark modes to maintain high contrast.
- [ ] Implement the `Spacebar` keyboard event listener to freeze and unfreeze the color selection, allowing the user to hover elsewhere without losing the locked color.
- [ ] Implement the click-to-copy functionality to write the current HEX color code to the clipboard, and display a temporary "Copied!" notification on the HUD.
- [ ] Set up a lightweight test suite (e.g. using a simple JS test runner or configuring Vitest/Jest if appropriate) to verify the core mathematical formulas:
